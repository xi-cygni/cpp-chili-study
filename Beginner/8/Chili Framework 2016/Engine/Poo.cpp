...
void Poo::Initializer(int in_x, int in_y, int in_vx, int in_vy)
{
	assert(initialized == false); // it will close the program if initialized was made true before
...
// we put these assertons below to make sure that noone will call these funcions before poos were initialized

bool Poo::TestCollision(const Dude& dude) const // dude variables
{
